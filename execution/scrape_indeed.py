#!/usr/bin/env python3
"""
Indeed Job Scraper Execution Script

This script uses the Apify Indeed Scraper actor to fetch job listings
...
Usage:
  python scrape_indeed.py "Software Engineer" "Product Manager"
"""
...
def run_apify_actor(job_titles: list[str], limit_per_title: int = 25) -> list[dict]:
    """Run the Apify Indeed actor and return the results."""
    ...
    for title in job_titles:
        print(f"Scraping Indeed jobs for: {title}")
        
        # Start the actor run
        run_url = f"https://api.apify.com/v2/acts/{INDEED_ACTOR_ID}/runs?token={APIFY_API_TOKEN}"
...
def save_to_supabase(jobs: list[dict]) -> int:
    ...
    for job in jobs:
        # Indeed jobs have different field names
        job_id = job.get('id') or job.get('positionId') or job.get('url', '').split('/')[-1]
        
        # Transform to our database schema
        db_job = {
            "external_id": f"indeed_{job_id}",
            "source": "indeed",
            "title": job.get('positionName') or job.get('title', ''),
